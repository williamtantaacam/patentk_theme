...
foreach($users as $user){
	...
	update_field('nickname', 'P' . combine_nums($first_num , $last_num , $email_num),  'user_' . $user -> ID);
	$toupdate = array('nickname' =>  'P' . combine_nums($first_num , $last_num , $email_num),  'user_' . $user -> ID);
	UM()->user()->update_profile($toupdate);
	
	um_fetch_user($user -> ID );
	$user_roles = get_userdata($user -> ID ) -> roles;
	
	if(in_array('subscriber', $user_roles) and count($user_roles) >= 2){
	...
	elseif(in_array('pms_subscription_plan_616', $user_roles)){
		$toupdate = array('testtt' => 'Premium Membership $79.99 / year + HST');
	}
